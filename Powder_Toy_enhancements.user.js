...
contentEval(function () {
    if (typeof $ != "undefined") // check jQuery has loaded
    {
        window.tptenhance = {
            ...
            isMod: function () {
                if (typeof tptenhance.isModCache != "undefined") return tptenhance.isModCache;
                tptenhance.isModCache = false;
                $(".main-menu .dropdown a.dropdown-toggle").each(function () {
                    if ($(this).text().indexOf("Admin") != -1) tptenhance.isModCache = true;
                });
            ...
            tagTooltip: function (element, tag, saveId) {
                ...
                $.get(getLocation, function (data) {
                    ...
                    var clickFunc = function (e) {
                            ...
                            var url = this.href;
                            var that = $(this);
                            if (that.text() == "Disable") that.replaceWith('<div class="pull-right label label-info" style="margin-right:10px;"><i class="icon-refresh icon-white"></i> <strong>Disabling...</strong></div>');
                            else that.replaceWith(tptenhance.deletingHtml);
                            $.get(url, function () {
                                element.remove(); // remove tag text
                                if (tptenhance.popoverSelectedTag == tag) tptenhance.removePopover(); // remove tag info popup
                                tptenhance.updatePopoverPosition();
                            });
                        };
                    content.find('div.TagInfo').each(function () {
            ...
            attachSaveCommentHandlers: function () {
                ...
                $(".Actions a").each(function () {
                    if (this.href.indexOf('DeleteComment=') != -1) $(this).click(clickFn);
                });
                $(".Pagination a").die('click');
                $(".Pagination a").on('click', function (e) {
                    e.preventDefault();
                    window.lastComments = this.href;
                    if (tptenhance.commentPageRequest) tptenhance.commentPageRequest.abort();
            ...
            tags: {
                ...
                disableButtonClick: function (e) {
                    ...
                    if (tptenhance.popoverSelectedTag == tag) tptenhance.removePopover();
...
*/
                parseReportsHtml: function (html) {
                    ...
                    $(html).find("li").each(function () {
                        reports.push(tptenhance.reports.parseReportsHtmlSingle($(this)));
                    });
/*reports.push({SaveId:17758,UnreadReportCount:2,SaveName:"8x6 line text display",Username:"jacksonmj"});
            ...
            saves: {
                ...
                showVotes: function () {
                    ...
                    var svg = d3.select("#VoteGraph").append("svg:svg").attr("width", w + m[1] + m[3]).attr("height", h + m[0] + m[2]).append("svg:g").attr("transform", "translate(" + m[3] + "," + m[0] + ")");

                    var gradient = svg.append("svg:defs").append("svg:linearGradient").attr("id", "gradient").attr("x2", "0%").attr("y2", "100%");

                    gradient.append("svg:stop").attr("offset", "0%").attr("stop-color", "#9ecae1").attr("stop-opacity", 0.5);

                    gradient.append("svg:stop").attr("offset", "100%").attr("stop-color", "#6baed6").attr("stop-opacity", 1);
                    ...
                    d3.json(tptenhance.saves.voteDataJsonUrl(currentSaveID), function (data) {
                        ...
                        if (data.dupVotes.length) {
                            ...
                            var dupVotes = data.dupVotes.sort(function (a, b) {
                            ...
                            dupVotes.forEach(function (d) {
                                    ...
                                    if (hoverTimeout === false) {
                                        hoverTimeout = setTimeout(function () {
                                            ...
                                            tptenhance.users.getModerationInfoById(d.UserID, function (data) {
                                                ...
                                                txt += "<br>";
                                                if (!data.Comments.length && !data.Tags.length) txt += "No tags or comments";
                                                else {
                                                    txt += data.Tags.length + " tags, ";
                                                    if (data.CommentPageCount > 1) txt += data.CommentPageCount + " pages of comments";
                                                    else if (data.Comments.length >= 10) txt += "many comments";
                                                    else txt += data.Comments.length + " comments";
                                                }
                                                // TODO: saves?
                                                that.tooltip({
                                                    title: txt,
                                ...
                                if (typeof ipcolours[d.SourceAddress] != "undefined" && ipcolours[d.SourceAddress] !== "") cell.css('background-color', ipcolours[d.SourceAddress]);
                                ...
                                if (iplist.length > 1) {
                                    tableRow.on("dblclick", function () {
                                        ...
                                        $(this).parents("tbody").find("tr").each(function () {
                                            if ($(this).find(".IPAddress a").text() == target) $(this).addClass("highlight");
                        ...
                        x.domain([d3.min(data.votes, function (d) {
                            return d.date;
                        }), d3.max(data.votes, function (d) {
                            return d.date;
                        })]);
                        var ydomain = d3.extent(data.votes, function (d) {
                            return d.value;
                        });
                        ...
                        function voteMouseover(d) {
                            ...
                            svg.selectAll(".dupVLine").classed("active", function (p) {
                                return p.SourceAddress === d.SourceAddress;
                            });
                        }
                        ...
                        dupVLine = voteLines.selectAll("line.link").data(data.dupVotes);

                        var lineG = dupVLine.enter().insert("svg:g").attr("class", function (d) {
                            return "dupVLine" + d.Vote + " dupVLine";
                        }).on("mouseover", voteMouseover).on("mouseout", voteMouseout);

                        lineG.append("line").attr("x1", 0).attr("x2", 0).attr("y1", h).attr("y2", -5);

                        lineG.append("text").attr("text-anchor", "middle").attr('font-size', 11).attr("dy", ".1em").text(function (d) {
                            return d.Username;
                        });

                        lineG.append("text").attr("text-anchor", "middle").attr('font-size', 11).attr("dy", ".1em").attr("transform", "translate(0, 14)").text(function (d) {
...
/*dupVLine.attr("x1", function(d) { return x(d.Date); })
                        ...
                        dupVLine.attr("transform", function (d) {
            ...
            users: {
                ...
                parseModerationPage: function (html) {
                    ...
                    data.KnownAddresses = [];
                    html.find(".KnownAddresses a").each(function () {
                        data.KnownAddresses.push($(this).text());
                    });
                    data.Comments = [];
                    ...
                    html.find(".BanHistory li").each(function () {
                        ...
                        if (duration.indexOf("permanently") != -1 || duration.indexOf("permenantly") != -1) duration = 0;
                        else if (duration.indexOf("hour") != -1) duration = 60 * 60 * (+duration.split(" ")[0]);
                        else if (duration.indexOf("day") != -1) duration = 60 * 60 * 24 * (+duration.split(" ")[0]);
                        else if (duration.indexOf("week") != -1) duration = 60 * 60 * 24 * 7 * (+duration.split(" ")[0]);
                        else if (duration.indexOf("month") != -1) duration = 60 * 60 * 24 * 7 * 4 * (+duration.split(" ")[0]); // 4 weeks seems right, e.g. a ban reported on IRC as 67200 hours shows as 100 months
                        otherText.pop();
                        data.Bans.push({
                            date: h6[0],
                            By: h6[1],
                    ...
                    data.Tags = [];
                    html.find(".TagText").each(function () {
...
/*var votesShown = false;
					votesTab.find("a").on("click", function(e){
                        ...
                        bumpsTab.find("a").on("click", function (e) {
                            ...
                            $.get(tptenhance.saves.infoJsonUrlPTT(currentSaveID), function (data) {
                                ...
                                if (data.BumpTimes.length) {
                                    if (data.BumpTimes.length > 1) $('<strong>This save has been bumped at least ' + data.BumpTimes.length + ' times:</strong>').appendTo(bumpList);
                                    else $('<strong>This save has been bumped at least once:</strong>').appendTo(bumpList);
                                    ...
                                    if (data.Signs.length) {
                                        data.Signs.forEach(function (s) {
                                            ...
                                            for (var i = 0; i < data.Signs.length; ++i) {
                                                if (data.Signs[i].PlacementX == s.PlacementX && data.Signs[i].PlacementY == s.PlacementY && data.Signs[i].RawText == s.RawText)++count;
                                            }
                                            if (count > 1) row.addClass("DupSign");
                        ...
                        searchesTab.find("a").on("click", function (e) {
                            tabSwitch(this);
                            var container = $('<div><strong>Search for similar saves by:</strong><br></div>').css({
                                "text-align": "center"
                            });
                            ...
                            if ($(".SaveDescription").text().trim() != "No Description provided.") {
                                $('<a></a>').attr('href', 'http://powdertoythings.co.uk/Powder/Saves/Search.html?Search_Query=' + encodeURIComponent("sort:id search:desc " + $(".SaveDescription").text().trim())).text("Description").append('<br>').appendTo(container);
                            }
                            tptenhance.saveDetailsTabContent.append(container);
                            e.preventDefault();
                        });
                        ...
                        $('<div></div>').append(tabs).css({
                            "text-align": "center"
                        }).appendTo(newDetailsPane);
            ...
            $(document).ready(function () {
                setTimeout(function () {
                    ...
                    $(".Pagination a").live('click', function () {
                        ...
                        if (matchesCurrent && matchesNew && (+matchesNew[1]) < (+matchesCurrent[1])) goBack = 1;
                        ...
                        $.get(Link, function (data) {
                            ...
                            $(".MessageListOuter").css({
                                "height": (+$("ul.MessageList").height() - OLHeight) + "px"
                            });
                            ProcessMessages();
                            doScroll();
        ...
        if (window.location.toString().indexOf("/Groups/Thread/") != -1) {
            $(document).ready(function () {
                ...
                $('.Post.Moderator').each(function () {
                    if ($(this).find(".Meta .UserTitle").text() == "Member") $(this).find(".Meta .UserTitle").text("Moderator");
                });
...
/* WIP, not finished yet
        ...
        if ($(".social-github iframe").length) $(".social-github iframe").attr("src", $(".social-github iframe").attr("src").replace("FacialTurd", "simtr"));
...
addCss('.Tag .DelButton, .Tag .UnDelButton { top:auto; background-color:transparent; }.Tag .LoadingIcon { position:absolute; right:3px; line-height:20px; }.popover-inner { width:380px; }.VoteUpIcon { background-color:#0C0; border:1px solid #080; }.VoteDownIcon { background-color:#C00; border:1px solid #800; }.VoteUpIcon, .VoteDownIcon { margin-top:2px; }.DupVotes { margin-top: 10px; }.DupVotes h4 { text-align:center; margin:3px 0; }.DupVotes table { margin:0 auto; border:1px solid #CCC; }.DupVotes td, .DupVotes th { padding:3px 6px; }.DupVotes th { text-align:left; background-color:#DDD; }.DupVotes tr:nth-child(even) { background-color:#FFF; }.DupVotes tr:nth-child(odd) { background-color:#EFEFEF; }.DupVotes tr:hover, .DupVotes tr.highlight:hover { background-color:#E0E0FF; }.DupVotes tr.highlight .IPAddress { background-color:#FFF !important; }.DupVotes tr.highlight { background-color:#C8C8FF; }.DupVotes .Date { font-family:monospace; }.SignsTbl { margin:0 auto; border:1px solid #CCC; }.SignsTbl td, .SignsTbl th { padding:3px 6px; border:1px solid #CCC}.SignsTbl th { text-align:left; background-color:#DDD; }.SignsTbl th:nth-child(2) { min-width:200px; }.SignsTbl td:nth-child(2), .SignsTbl td:nth-child(3) { text-align:left; }.SignsTbl tr:nth-child(even) { background-color:#FFF; }.SignsTbl tr:nth-child(odd) { background-color:#F9F9F9; }.SignsTbl tr:hover, .DupVotes tr.highlight:hover { background-color:#E0E0FF; }.SignsTbl tr.DupSign td:nth-child(1) { color:#C00; font-weight:bold; }.SignLinkSaveThumb { display:block; }.SignLinkSaveThumb img { clear:left; width:102px; height:64px; }.Post { word-wrap: break-word; }.savegame { width:153px; }.savegame .caption a { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.TagInfo { clear:right; }.TagInfo .label { margin-bottom:1px; }.SaveDetails ul.MessageList li.Post { border-top:1px solid #DCDCDC; border-bottom:0 none; }.new-topic-button .btn { white-space:nowrap; }');
if (window.location.toString().indexOf("/Groups/") != -1) {
    addCss('.TopicList li .TopicPages { width:auto; }.TopicList .Pagination li { padding:0; border-bottom: 1px solid #DCDCDC; line-height: normal; }.TopicList .Pagination a { font-size: 9px !important; line-height: 16px; min-width: 10px !important; padding: 0 3px; text-align: center; border-width: 1px 1px 1px 0 !important; }.TopicList .Pagination li:first-child a { border-left-width: 1px !important; }.TopicList .pagination { height: 16px; margin: 0; padding: 3px; }.contents h1 { font-size: 20px; }.GroupOptions { position:relative; top:0; right:0; float:right; clear:right;}.GroupDescription { margin:0; }.MessageListOuter { margin-bottom:7px; }.PostFForm #AddReplyMessage { width:100%; margin:0; padding:0; }.PostFForm, .ModerationFooter { margin:0; }.container { background: none repeat scroll 0 0 rgba(0, 0, 0, 0); border: medium none; padding: 0; }.Page { border: 1px solid #CDD2D7; }.Moderator .Author, .Administrator .Author { background-image: url("/Themes/Next/Design/Images/Shield.png"); }.main-menu li a[href="/Groups.html"] { display: none; }ul.MessageList li.Post div.Meta span.Actions2 { float:right; }ul.MessageList li.Post div.Meta span.Actions2 a { visibility:hidden; }ul.MessageList li.Post:hover div.Meta span.Actions2 a { visibility:visible; }.CurrentMembers .MemberActions select[name="Elevation"] { width:100px; }.MemberColumn { width:360px; }');
}
if (window.location.toString().indexOf("/Reports.html") != -1) {
    addCss('.container { background: none repeat scroll 0 0 rgba(0, 0, 0, 0); border: medium none; padding: 0; }.Page { border: 1px solid #CDD2D7; }.Subpage { background-color: #FFFFFF; padding:15px; }.contents h1 { font-size:20px; }/*ul.SaveReports, ul.SaveReports li { list-style:none outside none; margin:0; padding:0; border: 0 none; background-color:#FDFDFD; }.SaveReports .Save { border:1px solid #999; border-radius: 3px; margin:10px 0; }.SaveReports .Save .badge { float:right; margin:0 5px;}.SaveReports .Save .Actions .btn { margin:5px 10px 0 10px; min-width:100px;}.SaveReports .Save .Actions { padding:0 10px;  text-align:center; }.SaveReports .SaveThumb { float:left; height:128px; width:204px; border-radius:0 3px 0 0; margin:5px 10px 5px 5px;}.SaveReports .SaveTitleContainer { font-size:16px; font-weight:bold; border-radius:3px 0 0 0; border:0 none; margin:0; padding:10px; border-bottom:1px solid #DDDDDD; }.SaveReports .SaveDetails { font-size:15px; overflow:hidden;margin-bottom:15px; }.SaveReports .SaveDetails .btn { margin:0 3px; float:right; }.SaveReports .SaveDetails .SaveId, .SaveReports .SaveDetails .SaveAuthor { font-weight:bold; }.SaveReports .Save .DetailsContainer { padding:10px; }*//*.SaveReports img { height: 96px; width:153px; margin:5px;  }*/.SaveReports .MainInfo { width:auto !important; }.SaveReports li > span { margin:0 5px 0 5px !important; }');
}
